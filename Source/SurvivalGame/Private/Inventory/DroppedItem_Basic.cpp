...
void ADroppedItem_Basic::BeginInteraction_Implementation(const FInteractionInfo Info)
{
	...
	UUtilityFunctionLibrary::PrintDebug("Attempting to pick up item");
	int32 Remainder = -1;
...
void ADroppedItem_Basic::BeginInteraction_Implementation(const FInteractionInfo Info)
{
	...
	InteractingControllerRef = Info.ControllerRef;
	UInventoryComponent* InventoryRef = Info.PawnRef->FindComponentByClass<UInventoryComponent>();
...
void ADroppedItem_Basic::UpdateGraphics_Implementation()
{
	//UUtilityFunctionLibrary::PrintDebug("UpdateGraphics_Implementation");
	UBasicItemSlotWidget* DisplayWidget;
	if (IsValid(ItemDataDisplayComponent1)) {
		//UUtilityFunctionLibrary::PrintDebug("Display is Valid");
		if (IsValid(ItemDataDisplayComponent1->GetUserWidgetObject())) {
			//UUtilityFunctionLibrary::PrintDebug("Display widget is Valid");
			DisplayWidget = Cast<UBasicItemSlotWidget>(ItemDataDisplayComponent1->GetUserWidgetObject());
			if (DisplayWidget) {
				//UUtilityFunctionLibrary::PrintDebug("Cast Widget is Valid");
	...
	if (ItemDataDisplayComponent2 && IsValid(ItemDataDisplayComponent2) && IsValid(ItemDataDisplayComponent2->GetUserWidgetObject())) {
		DisplayWidget = Cast<UBasicItemSlotWidget>(ItemDataDisplayComponent2->GetUserWidgetObject());
		if (DisplayWidget) {
			DisplayWidget->SetItemData(ItemData);
			//DisplayWidget->UpdateGraphics();
		}
